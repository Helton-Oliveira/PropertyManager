...
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
        IF NEW.role = 'TENANT' THEN
            INSERT INTO tenants (user_id, name, cpf, email, password, phone, active)
            VALUES (NEW.id, NEW.name, NEW.cpf, NEW.email, NEW.password, NEW.phone, TRUE);
        ELSIF NEW.role = 'OWNER' THEN
            INSERT INTO owners (user_id, name, cpf, email, password, phone, active)
            VALUES (NEW.id, NEW.name, NEW.cpf, NEW.email, NEW.password, NEW.phone, TRUE);
        END IF;
    END IF;
